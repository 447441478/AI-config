...
(function () {
  ...
  var GLOBAL_KEY = 'HeiGuiSaltLocker',
    VERSION = "20260914-sort-arrival-enhanced",
  ...
  /* =========================================================
   * 增强模块 1：战力与精力数值深度解析（递归探测支持深层嵌套与别名）
   * ========================================================= */
  ...
  function readEnemyPower(role, cached) {
    return readNumberField({ role: role, cached: cached }, [
      'power',
      'fightPower',
      'combatPower',
      'battlePower',
  ...
  function getPlayerMarchRemainSeconds(player) {
    ...
    if (!endTime) return 0;
    if (endTime > 0 && endTime < 100000000000) endTime *= 1000;
  ...
  /* =========================================================
   * 增强模块 3：部队列表排序核心与 FairyGUI 原生矢量按钮栏
   * ========================================================= */
  ...
  function makeNativeSortButton(fg, text, onClick) {
    if (!fg) fg = getFgui();
  ...
  function ensureNativeTroopsSortButtons(page) {
    ...
    if (!page.__sfaSortBar) {
      ...
      label.verticalAlign = 'middle';
      label.touchable = false;
      setNodeSize(label, 56, 42);

      var energy = makeNativeSortButton(fg, '精力降序', function () {
        state.sortMode = 'energy';
        state.sortOrder = 'desc';
        updateNativeSortButtons(page);
        sortTroopsList(page);
      });
      var power = makeNativeSortButton(fg, '战力降序', function () {
      ...
      ui.addChild(energy);
      ui.addChild(power);
      page.__sfaSortBar = { label: label, energy: energy, power: power };
      if (Array.isArray(state.sortBars)) {
        state.sortBars.push(page.__sfaSortBar);
      }
    }

    var y = Math.max(0, Number(list ? list.y : 270) - 54);
    var right = Number(ui.width || 720) - 40;
    var powerX = Math.max(420, right - 115);
    var energyX = powerX - 125;
    var labelX = energyX - 64;
