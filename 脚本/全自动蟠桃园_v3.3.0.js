...
// @version      3.3.0
// @description  咸鱼之王全自动蟠桃园(运镖/截镖)挂机助手，支持自动进场、复活布阵、上船护船、终点自动下船(无人驾驶)、精确同船/地面索敌攻击与道具拾取，兼容单庞统与主阵容。
...
(() => {
  ...
  (function() {
    ...
    function getRoleCarId(role) {
      return getId(role, "carId", "targetCarId", "currentCarId", "bindCarId");
    }
    function isRoleInCarQueues(roleId, car) {
      var _a, _b;
      if (!roleId || !car) return false;
      if (car.memberMap) {
        if (typeof car.memberMap.has === "function") {
          if (car.memberMap.has(roleId)) return true;
        } else if (typeof car.memberMap === "object" && car.memberMap[roleId]) {
          return true;
        }
      }
      const queues = [car.friendQueue, car.enemyQueue, car.battleQueue, car.members, car.memberList, car.roles];
      for (const list of queues) {
        if (Array.isArray(list) && list.some((item) => getId(item, "roleId", "id") === roleId)) {
          return true;
        }
      }
      return false;
    }
    function isRoleOnCar(role, car, bf) {
      if (!role || !car) return false;
      const roleId = getId(role, "roleId", "id");
      const carId = getId(car, "carId", "id");
      if (!roleId || !carId) return false;

      // 1. 直接比对角色绑定的 carId
      const roleCarId = getRoleCarId(role);
      if (roleCarId && String(roleCarId) === String(carId)) return true;

      // 2. 检查特定船只的成员队列
      if (isRoleInCarQueues(roleId, car)) return true;

      // 3. 兜底策略：仅当 role.isOnCar 为 true 且场上仅有一艘活跃船只时才归属该船
      if (role.isOnCar && bf) {
        const cars = getCars(bf);
        if (cars.length === 1 && String(getId(cars[0], "carId", "id")) === String(carId)) {
          return true;
        }
      }
      return false;
    }
    function getRoleCar(bf, role) {
      if (!bf || !role) return null;
      const cars = getCars(bf);
      if (!cars.length) return null;

      // 1. 优先通过角色自身的 carId 字段精确定位
      const roleCarId = getRoleCarId(role);
      if (roleCarId) {
        const matched = cars.find((c) => String(getId(c, "carId", "id")) === String(roleCarId));
      ...
      if (roleId) {
        for (const car of cars) {
          if (isRoleInCarQueues(roleId, car)) return car;
        }
      }

      // 3. 兜底：若 role.isOnCar 为 true 且场上仅有一艘船
      if (role.isOnCar && cars.length === 1) {
        return cars[0];
      }
    ...
    function pickBestCar(bf, self, lp) {
      ...
      const onCar = cars.find((car) => isRoleOnCar(self, car, bf));
    ...
    /**
     * 精确目标筛选：
     * 1. 若自己在船上，目标必须同在这一艘船上（严禁攻击陆地或其它船只上的敌人）。
     * 2. 若自己在地面，目标必须同样在地面（严禁从地面攻击船上敌人）。
     */
    function pickBestEnemy(bf, self, currentCar) {
      if (!bf || !self) return null;
      // 1. 确定自己当前所在船
      const selfCar = getRoleCar(bf, self) || (currentCar && isRoleOnCar(self, currentCar, bf) ? currentCar : null);
      const selfCarId = selfCar ? getId(selfCar, "carId", "id") : null;

      const enemies = getRoles(bf).filter((role) => isEnemy(self, role)).map((role) => {
        // 2. 确定该敌人当前所在船
        const enemyCar = getRoleCar(bf, role) || (selfCar && isRoleOnCar(role, selfCar, bf) ? selfCar : null);
        const enemyCarId = enemyCar ? getId(enemyCar, "carId", "id") : null;

        // 3. 严格的环境一致性校验：
        // 若自己在船上，目标必须同在此船上！
        if (selfCarId) {
          if (!enemyCarId || String(enemyCarId) !== String(selfCarId)) {
            return null; // 敌人在岸上或在其他船上，坚决不选！
          }
        } else {
          // 若自己在地面，目标必须同样在地面！
          if (enemyCarId) {
            return null; // 敌人在船上，陆地不能直接打！
          }
        }

        // 4. 距离判定：同船距离视为 0；同在地面计算坐标距离
        const dist = selfCarId ? 0 : distance(self, role);
        return {
          role,
          dist,
          carDist: currentCar ? distance(role, currentCar) : Infinity,
          onCar: !!enemyCarId
        };
      }).filter((x) => x && x.dist <= CFG.attackRange);
      ...
      if (CFG.strategy.priorityTarget === "nearCar" && currentCar) {
    ...
    function tick() {
      ...
      try {
        ...
        const isSelfOnCar = !!(state.currentCar && isRoleOnCar(self, state.currentCar, bf));
        ...
        if (CFG.strategy.autoGetOnCar && state.currentCar && !isRoleOnCar(self, state.currentCar, bf)) {
          const carId = getId(state.currentCar, "carId", "id");
        ...
        if (CFG.strategy.autoAttack && shouldRun(state.lastBattle, CFG.battleCooldownMs, now)) {
          // 下船防抖保护：刚下船1秒内避免因状态同步延迟发起战斗
          if (shouldRun(state.lastDismountAt, 1000, now)) {
    ...
    function getDetailedStatus() {
      ...
      return {
        ...
        reviveSeconds,
        isOnCar: !!(car && isRoleOnCar(self, car, bf)),
    ...
    window.__LP_SMART__ = {
      ...
      attackNearest() {
        ...
        const enemyId = getId(enemy, "roleId", "id");
        if (!(lp == null ? void 0 : lp.sendBattle) || !enemyId) return addTask("error", "攻击范围内无合法目标(需同在船上或同在陆地)");
        state.battleCount++;
